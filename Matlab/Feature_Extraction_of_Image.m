...
baseFileName = 'long pulse -5s01_export1266.JPG'; % Assign the one on the button that they clicked on.
...
set(gcf, 'Name', 'Output Results', 'NumberTitle', 'Off') 
% Get the color map.
...
subplot(2, 3, 5);
meanTemp=mean2(thermalImage);
imshow(thermalImage, []);
axis on;
colorbar;
%title('Floating Point Thermal (Temperature) Image', 'FontSize', fontSize, 'Interpreter', 'None');
title('depth of defect(in cm)', 'FontSize', fontSize, 'Interpreter', 'None');
...
axis on;
grid on;
caption = sprintf('Histogram of depth of defect Image');
title(caption, 'FontSize', fontSize, 'Interpreter', 'None');
xlabel('Depth(in cm)', 'FontSize', fontSize, 'Interpreter', 'None');
